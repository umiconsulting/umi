...
function Kpi({ label, value, delta, deltaTone, meta }) {
  return (
    <div style={{ border: '1px solid var(--line)', borderRadius: 14, padding: '14px 16px', background: 'var(--surface)', display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div className="figures" style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, fontSize: 24, letterSpacing: '-0.02em', lineHeight: 1 }}>
        {value}
      </div>
      <div style={{ fontSize: 11.5, color: 'var(--ink-3)', display: 'flex', gap: 8, alignItems: 'center' }}>
        {delta ? (
          <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, color: deltaTone }}>{delta}</span>
        ) : null}
...
function SeriesChart({ series, bucket, currency }) {
  ...
  const peak = series.reduce((b, p, i) => (p.amountMinorUnits > series[b].amountMinorUnits ? i : b), 0);
  return (
    <section style={{ border: '1px solid var(--line)', borderRadius: 14, background: 'var(--surface)', padding: '16px 18px' }}>
      <div className="eyebrow" style={{ marginBottom: 12 }}>
        {bucket === 'hour' ? <Trans>Venta por hora</Trans> : <Trans>Venta por día</Trans>}
      </div>
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 150, overflowX: 'auto', borderBottom: '1px solid var(--line)', paddingBottom: 2 }} role="img" aria-label={t`Serie de ventas del periodo`}>
        {series.map((p, i) => (
          <div key={p.label} style={{ flex: '1 0 22px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            <div style={{ flex: 1, width: '100%', display: 'flex', alignItems: 'flex-end' }}>
              <div
                title={`${p.label} · ${formatOperationMoney(p.amountMinorUnits, currency)}`}
                style={{ width: '100%', height: `${Math.max((p.amountMinorUnits / max) * 100, 2)}%`, minHeight: 2, borderRadius: '5px 5px 0 0', background: i === peak ? 'var(--merchant-brand)' : 'color-mix(in srgb, var(--merchant-brand) 22%, transparent)' }}
              />
            </div>
  ...
  return (
    <section style={{ border: '1px solid var(--line)', borderRadius: 14, background: 'var(--surface)', padding: '16px 18px' }}>
      ...
      <div style={{ display: 'flex', height: 12, borderRadius: 6, overflow: 'hidden', marginBottom: 14 }}>
        {paymentMix.map((p) => (
          <div key={p.method} style={{ width: `${(p.amountMinorUnits / total) * 100}%`, background: METHOD_COLOR[p.method] || 'var(--ink-3)' }} />
        ))}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
        {paymentMix.map((p) => (
          <div key={p.method} style={{ display: 'flex', alignItems: 'center', gap: 9, fontSize: 13.5 }}>
            <span aria-hidden="true" style={{ width: 9, height: 9, borderRadius: '50%', background: METHOD_COLOR[p.method] || 'var(--ink-3)', flexShrink: 0 }} />
            <span style={{ flex: 1 }}>{i18n._(METHOD_LABEL[p.method] || p.method)}</span>
            <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--ink-3)', width: 46, textAlign: 'right' }}>
              {Math.round((p.amountMinorUnits / total) * 100)}%
            </span>
            <span className="figures" style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, width: 110, textAlign: 'right' }}>
...
function Pivot({ productMix, byOperator, byHour, currency }) {
  ...
  const rows =
    groupBy === 'producto'
      ? productMix.map((p) => ({ key: p.productName, sub: p.category, units: p.units, net: p.netMinorUnits }))
      : groupBy === 'categoria'
        ? Object.values(
  ...
  const sorted = [...rows].sort((a, b) =>
    sort.col === 'key' ? mul * String(a.key).localeCompare(b.key, 'es') : mul * (a[sort.col] - b[sort.col]),
  );
  const th = (col, node, alignRight) => {
    const active = sort.col === col;
    return (
      <th
        onClick={() => setSort((s) => (s.col === col ? { col, dir: s.dir === 'asc' ? 'desc' : 'asc' } : { col, dir: col === 'key' ? 'asc' : 'desc' }))}
  ...
  return (
    <section style={{ border: '1px solid var(--line)', borderRadius: 14, background: 'var(--surface)', overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: '1px solid var(--line)', flexWrap: 'wrap' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5, minWidth: 480 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--line)', fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              ...
              {groupBy === 'producto' ? <th style={{ padding: '9px 12px', textAlign: 'left', fontWeight: 600, color: 'var(--ink-3)' }}><Trans>Categoría</Trans></th> : null}
              {th('units', <Trans>Unidades</Trans>, true)}
              {th('net', <Trans>Venta neta</Trans>, true)}
              <th style={{ padding: '9px 12px', textAlign: 'right', fontWeight: 600, color: 'var(--ink-3)' }}><Trans>% del total</Trans></th>
            </tr>
          </thead>
          <tbody style={{ fontFamily: 'var(--font-mono)' }}>
            {sorted.map((r) => (
              <tr key={r.key} style={{ borderTop: '1px solid var(--line-soft)' }}>
                <td style={{ padding: '9px 12px', fontFamily: 'var(--font-sans, inherit)', fontWeight: 500 }}>{r.key}</td>
                {groupBy === 'producto' ? <td style={{ padding: '9px 12px', color: 'var(--ink-3)', fontFamily: 'var(--font-sans, inherit)' }}>{r.sub || '—'}</td> : null}
                <td style={{ padding: '9px 12px', textAlign: 'right' }}>{r.units}</td>
                <td style={{ padding: '9px 12px', textAlign: 'right' }}>{formatOperationMoney(r.net, currency)}</td>
                <td style={{ padding: '9px 12px', textAlign: 'right' }}>{Math.round((r.net / pctDenom) * 100)}%</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr style={{ borderTop: '2px solid var(--line-strong)', fontWeight: 700, background: 'var(--surface-2)' }}>
              <td style={{ padding: '9px 12px' }}>
                <Trans>Total</Trans>
              </td>
              {groupBy === 'producto' ? <td /> : null}
              <td style={{ padding: '9px 12px', textAlign: 'right', fontFamily: 'var(--font-mono)' }}>{totalUnits}</td>
              <td style={{ padding: '9px 12px', textAlign: 'right', fontFamily: 'var(--font-mono)' }}>{formatOperationMoney(totalNet, currency)}</td>
              <td style={{ padding: '9px 12px', textAlign: 'right', fontFamily: 'var(--font-mono)' }}>100%</td>
            </tr>
          </tfoot>
        </table>
      </div>
...
export default function VentasReport() {
  ...
  const deltaTone = delta == null ? 'var(--ink-3)' : delta >= 0 ? 'var(--success)' : 'var(--danger)';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {view !== 'recibos' ? (
          <div className="tabbar" role="group" aria-label={t`Rango de fechas`} style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {RANGES.map((r) => (
              <button key={r.id} type="button" className={'btn btn-sm ' + (range === r.id ? 'btn-primary' : 'btn-secondary')} aria-pressed={range === r.id} onClick={() => setRange(r.id)}>
                {i18n._(r.label)}
              </button>
            ))}
          </div>
        ) : null}
        <div role="group" aria-label={t`Vista`} style={{ display: 'flex', gap: 6, marginLeft: 'auto' }}>
          <button type="button" className={'btn btn-sm ' + (view === 'resumen' ? 'btn-primary' : 'btn-secondary')} aria-pressed={view === 'resumen'} onClick={() => setView('resumen')}>
            <Trans>Resumen</Trans>
          </button>
          <button type="button" className={'btn btn-sm ' + (view === 'pivot' ? 'btn-primary' : 'btn-secondary')} aria-pressed={view === 'pivot'} onClick={() => setView('pivot')}>
            <Trans>Tabla dinámica</Trans>
          </button>
          <button type="button" className={'btn btn-sm ' + (view === 'recibos' ? 'btn-primary' : 'btn-secondary')} aria-pressed={view === 'recibos'} onClick={() => setView('recibos')}>
            <Trans>Recibos</Trans>
          </button>
        </div>
      </div>
      ...
      {view === 'recibos' ? (
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12 }}>
            ...
            />
            <Kpi label={<Trans>Órdenes</Trans>} value={totals?.orders ?? 0} meta={<Trans>en el periodo</Trans>} />
            <Kpi label={<Trans>Ticket promedio</Trans>} value={formatOperationMoney(totals?.averageTicketMinorUnits ?? 0, currency)} meta={<Trans>por orden</Trans>} />
            <Kpi label={<Trans>Ítems vendidos</Trans>} value={totals?.itemsSold ?? 0} meta={<Trans>en el periodo</Trans>} />
            <Kpi label={<Trans>Descuentos</Trans>} value={formatOperationMoney(totals?.discountMinorUnits ?? 0, currency)} meta={<Trans>en el periodo</Trans>} />
            <Kpi label={<Trans>Propinas</Trans>} value={formatOperationMoney(totals?.tipsMinorUnits ?? 0, currency)} meta={<Trans>en el periodo</Trans>} />
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.6fr) minmax(0, 1fr)', gap: 16 }} className="ventas-grid">
