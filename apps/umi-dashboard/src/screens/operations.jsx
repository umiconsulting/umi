...
function HardwareDialog({ row, onClose, onComplete }) {
  const command = useAdministrativeCommand();
  const [message, setMessage] = useState('');
  ...
  async function execute(operation) {
    const response = await command.execute(operation, row.id, {
      targetVersion: row.version,
      parameters: {
        hardwareId: row.id,
        expectedConfigurationVersion: row.version,
        diagnostic: operation === 'hardware.diagnostic' ? 'connection_test' : null,
        sourceAggregateType: 'dashboard_hardware_test',
        sourceAggregateId: row.publicReference,
      },
    });
    ...
    setMessage(`Comando ${result.command.status}. Referencia ${result.command.commandId}.`);
    onComplete();
  }

  async function assign() {
    const response = await command.execute('hardware.assign', row.id, {
      targetVersion: row.version,
      parameters: {
        expectedVersion: row.version,
        registerId: registerId || null,
        assignedPosDeviceId: executorId || null,
        primary,
      },
    });
    setMessage(`Asignación guardada. Versión ${response.result.optimisticVersion}.`);
    onComplete();
  }

  async function setEnabled(enabled) {
    const response = await command.execute('hardware.update', row.id, {
      targetVersion: row.version,
      parameters: { expectedVersion: row.version, enabled },
    });
    setMessage(enabled ? 'Hardware habilitado.' : 'Hardware deshabilitado.');
    onComplete(response.result);
  }

  return (
    <div className="modal-backdrop" role="presentation">
      <section className="card modal-card" role="dialog" aria-modal="true" aria-label="Hardware">
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <h3 style={{ margin: 0 }}>Hardware: {row.title}</h3>
          <button className="btn-icon" type="button" onClick={onClose} aria-label="Cerrar">
            ×
          </button>
        </div>
        <p>La API entrega el comando al POS asignado. El navegador no accede al equipo.</p>
        <label>
          Registro
          <input
            value={registerId}
        ...
        <label>
          ...
          <input
            ...
            placeholder="ID del POS inscrito"
          />
        </label>
        <label>
          <input
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button
            className="btn"
            type="button"
            disabled={command.pending || !executorId}
            onClick={assign}
          >
            Guardar asignación
          </button>
          <button
            ...
            type="button"
            disabled={command.pending}
            onClick={() => setEnabled(row.status === 'disabled')}
          >
            {row.status === 'disabled' ? 'Habilitar' : 'Deshabilitar'}
          </button>
          <button
            className="btn"
            type="button"
            disabled={command.pending}
            onClick={() => execute('hardware.diagnostic')}
          >
            Diagnóstico
          </button>
          <button
            ...
            type="button"
            disabled={command.pending}
            onClick={() => execute('hardware.printer.test')}
          >
            Página de prueba
          </button>
        </div>
...
export default function OperationsScreen() {
  ...
  const [hardwareRow, setHardwareRow] = useState(null);
