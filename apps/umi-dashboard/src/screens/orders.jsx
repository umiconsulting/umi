...
const OrdersScreen = () => {
  ...
  const totalRevenue = allTickets
    .filter(function (t) {
      return t.status === 'completed' || ACTIVE_STATUSES.indexOf(t.status) !== -1;
    })
    .reduce(function (s, t) {
      return s + (parseFloat(t.total_amount) || 0);
    }, 0);
    ...
    >
      ...
      >
        ...
        <div style={{ minWidth: 120 }}>
          ...
          <span className="chip" style={{ fontSize: 11.5, height: 24, fontWeight: 600 }}>
            {ticket.station_name || '—'}
          </span>
        </div>
        ...
        <div style={{ minWidth: 90, textAlign: 'right', marginLeft: 'auto' }}>
          ...
          >
            {ticket.total_amount
              ? '$ ' + parseFloat(ticket.total_amount).toLocaleString('es-MX')
              : '—'}
          </div>
        ...
        <div style={{ minWidth: 52, textAlign: 'right', flexShrink: 0 }}>
          ...
          >
            {fmtAgo(ticket.created_at)}
          </div>
        </div>

        <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
          {ticket.status === 'new' && (
            <button className="btn btn-secondary btn-sm" onClick={() => onTransition('accepted')}>
              Aceptar
            </button>
          )}
          ...
          <button className="btn-icon" onClick={onSelect} aria-label="Ticket detail">
        ...
        <div className="sheet-head">
          <div>
            ...
            <h2 className="h-section" style={{ marginTop: 4 }}>
              {ticket.customer_name || 'Pedido WhatsApp'}
            </h2>
          </div>
        ...
        <div className="sheet-body">
          <div className="card" style={{ padding: 16 }}>
            ...
            {ticket.customer_note && (
              <div style={{ marginTop: 10, color: 'var(--ink-2)' }}>{ticket.customer_note}</div>
            )}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {items.length === 0 ? (
              <div style={{ color: 'var(--ink-3)', fontSize: 13 }}>
                No hay items disponibles para este ticket.
              </div>
            ) : (
              items.map((item) => (
        ...
        <div className="sheet-foot">
          ...
          {ticket.status !== 'completed' && ticket.status !== 'cancelled' && (
            <button
              ...
              onClick={() => onTransition(nextStatus(ticket.status))}
            >
              Avanzar estado
            </button>
          )}
