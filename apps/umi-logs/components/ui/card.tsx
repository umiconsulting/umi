...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "text-card-foreground flex flex-col gap-3 rounded-none border-b border-[var(--ruled-line)] py-3",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-[10px] uppercase tracking-wider leading-none", className)}
      {...props}
    />
  )
}
