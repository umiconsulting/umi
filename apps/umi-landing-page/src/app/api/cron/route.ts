...
export async function GET() {
  try {
    ...
    const errorMessage = cronError instanceof Error ? cronError.message : 'Error desconocido';
  ...
  try {
    ...
    switch (action) {
      ...
      case 'stop_job':
        ...
        return NextResponse.json({
          success: stopResult,
          message: stopResult ? `Job ${jobName} detenido` : `Job ${jobName} no encontrado`,
          timestamp: new Date().toISOString(),
        });
    ...
    const errorMessage = actionError instanceof Error ? actionError.message : 'Error desconocido';
