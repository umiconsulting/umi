...
describe('LeadDatabase', () => {
    ...
    test('Debe actualizar lead existente', () => {
      ...
      const updatedLead: LeadData = {
        ...
        diagnosticData: {
          ...
          score: 85,
          level: 'Avanzado',
          recommendations: ['Implementar análisis predictivo', 'Automatizar procesos de reporting'],
        } as DiagnosticData,
      };
  ...
  describe('Gestión de emails', () => {
    const testLead: LeadData = {
      ...
      diagnosticData: {
        score: 60,
        level: 'Básico',
        recommendations: ['Establecer procesos básicos de recolección de datos'],
        areas: {
          dataCollection: 50,
          analysis: 60,
          visualization: 65,
  ...
  describe('Leads pendientes', () => {
    test('Debe identificar leads pendientes de emails', () => {
      ...
      const pendingLead = pendingLeads.find((lead) => lead.id === 'test-lead-pending');
    ...
    test('No debe incluir leads pausados', () => {
      ...
      const pendingLeads = database.getLeadsPendingEmails();
      const pausedLead = pendingLeads.find((lead) => lead.id === 'test-lead-paused');
