...
const ORDER_CONFIRMATION_TOOLS = new Set([
  "confirm_order",
  "reorder_last_order",
]);
...
export function applyToolOutcome(
  ...
  if (ORDER_CONFIRMATION_TOOLS.has(toolName)) {
    state.orderConfirmed = true;
  }

  if (toolName === "confirm_order_changes") {
    state.orderChangesConfirmed = true;
  }
  ...
  if (suggestedTemplate) {
    state.suggestedTemplate = suggestedTemplate;
  }
}
...
function extractSuggestedTemplate(
  ...
  if (toolName === "add_to_cart" || toolName === "edit_cart") {
    return result?.summary_text ?? result?.customer_reply ?? null;
  }

  if (
    ORDER_CONFIRMATION_TOOLS.has(toolName) ||
    toolName === "confirm_order_changes" || toolName === "cancel_order"
  ) {
    return result?.customer_reply ?? result?.message ?? null;
  }
