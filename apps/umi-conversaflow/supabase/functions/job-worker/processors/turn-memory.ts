import type { SemanticResult, WorkingMemory } from "../../_shared/memory.ts";

export const DEFAULT_MAX_RECENT_MESSAGES = 8;
export const DEFAULT_MAX_RETRIEVED_MEMORIES = 5;

export type TurnMemoryMetadata = {
  recent_count: number;
...
export function shapeTurnMemory(
  ...
  const maxRecentMessages = options.maxRecentMessages ??
    DEFAULT_MAX_RECENT_MESSAGES;
  const maxRetrievedMemories = options.maxRetrievedMemories ??
    DEFAULT_MAX_RETRIEVED_MEMORIES;

  const shaped: WorkingMemory = {
    ...workingMemory,
    recentMessages: workingMemory.recentMessages.slice(-maxRecentMessages),
    semanticContext:
      workingMemory.semanticContext?.slice(0, maxRetrievedMemories) ?? null,
  };
  ...
  if (!shaped.semanticContext?.length) {
    omittedReasons.push("no_semantic_recall");
  }
