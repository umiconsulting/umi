import type { VoiceConfig } from "../_shared/business-config.ts";
import type { CustomerFacts, WorkingMemory } from "../_shared/memory.ts";
import type { PartialCancelledOrderContext } from "./context.ts";
...
function buildWorkingMemorySections(ctx: PromptContext): string {
  ...
  const factsSection = wm?.facts && Object.keys(wm.facts).some((k) => {
      const v = (wm.facts as any)[k];
      return Array.isArray(v) ? v.length > 0 : v != null;
    })
    ? `\n## HISTORIAL DEL CLIENTE\n${
      formatFacts(wm.facts)
...
Items cancelados:
${
      ctx.partialCancelledOrder.cancelledItems.map((item) =>
...
Items restantes:
${
      ctx.partialCancelledOrder.remainingItems.map((item) =>
        `- ${item.quantity}x ${item.name}${
          item.variantName ? ` (${item.variantName})` : ""
        }`
      ).join("\n") || "- Ninguno"
