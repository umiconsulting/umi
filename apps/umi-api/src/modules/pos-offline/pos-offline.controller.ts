...
import {
  AcknowledgeReconciliationRequest, BeginReplayRequest, ReconcileRequest,
  ReplayBatch, ReplayContextQuery,
} from '@umi/contract';
...
export class PosOfflineController {
  ...
  reconcile(
    @CurrentUser() user: AuthUser, @Param('tenantId') tenantId: string,
    @Query('branchId') branchId: string, @Query('operatorSessionId') operatorSessionId: string,
    ...
    return this.offline.reconcile(
      user, tenantId, branchId, operatorSessionId, Number(credentialVersion), dto,
    );
  }
