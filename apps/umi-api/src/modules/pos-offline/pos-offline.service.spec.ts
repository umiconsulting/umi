...
const command = (sequence: number): OfflineCommand => {
  ...
  branchId: uid(5), operatorSessionId: uid(6), commandType: 'operational.ack' as const,
...
describe('PosOfflineService', () => {
  it('issues a default-deny policy when no branch policy exists', async () => {
    const repo = {
      ...
      context: vi.fn(async () => ({
        lifecycle: 'active', credentialVersion: 1, deviceKind: 'pos_terminal',
        currency: 'MXN', permissions: ['offline.replay', 'offline.cash.checkout'],
        entitlements: [{ featureKey: 'pos.offline_cash', enabled: true }],
        lastAcceptedSequence: 0,
      })),
      policy: vi.fn(async () => null),
    } as unknown as PosOfflineRepository;
    const value = await new PosOfflineService(repo, checkout).issuePolicy(
      user, uid(4), {
        branchId: uid(5), operatorSessionId: uid(6), credentialVersion: 1,
      },
    );
  ...
  it('issues a bounded fingerprinted cash policy from server configuration', async () => {
    ...
    const repo = {
      ...
      policy: vi.fn(async () => ({
        id: uid(70), enabled: true, version: 'policy-1', currency: 'MXN',
        maxPolicyAgeSeconds: 600, maxSingleSaleMinorUnits: '10000',
        maxAccumulatedMinorUnits: '30000', maxOfflineSaleCount: 3,
        maxActiveQueueDepth: 10, maxCommandAgeSeconds: 3600,
        maxCatalogAgeSeconds: 900, maxPricingAgeSeconds: 600,
        maxTaxAgeSeconds: 600, managerApprovalThresholdMinorUnits: null,
        allowedDeviceClasses: ['pos_terminal'], issuedAt: now,
  ...
  it('fails closed on cross-tenant envelopes', async () => {
    ...
    await expect(new PosOfflineService(repo, checkout).batch(user, uid(99), {
