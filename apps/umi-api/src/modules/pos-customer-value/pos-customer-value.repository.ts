  ...
  async issueGiftCard(
    ...
    const fundingFingerprint =
      dto.source === 'sale'
        ...
        : null;
    // Uppercase hex, not base64url: build-v3's `masked_code` CHECK requires the shown
    // suffix to be `[A-Z0-9]{4}`, and the suffix is the code's last four characters.
    // 18 bytes is 144 bits either way.
    const code = `UMI-${randomBytes(18).toString('hex').toUpperCase()}`;
    ...
    const card = await client.query<Row>(
      // The clear code is never stored (build-v3, #128). Digest and display-safe
      // suffix are computed here, exactly as the Cash register does.
      `INSERT INTO merchant.loyalty_gift_card(
        id,merchant_id,code_hash,masked_code,public_reference,status,currency,amount_cents,
       ...
       VALUES($1::uuid,$2::uuid,extensions.digest($3,'sha256'),'••••-'||right($3,4),
        'GFT-'||$1::text,$4,$5,$6,$7::uuid,$8::uuid,$9::uuid,$10,
