 ...
 * merchant_id predicates (both callers — cash self-registration + WhatsApp ingress —
 ...
 * phone," reachable through one or more channels. `merchant.contact` is one reachability
 ...
 * on `(merchant_id, normalized_value)` across the phone family reaches one customer.
 */
@Injectable()
export class IdentityResolver {
  ...
  private async resolveWithin(c: PoolClient, input: ResolveInput): Promise<ResolvedIdentity> {
    const { merchantId, channelKey } = input;
    ...
    // createCustomer AND ensureContact both do find-then-write, and merchant.contact
    ...
    await c.query('SELECT pg_advisory_xact_lock(hashtext($1)::bigint)', [
      `${merchantId}:${normalized ?? `${channel.id}:new`}`,
    ]);
    let found =
      normalized == null
        ? null
        : await this.findCustomer(c, merchantId, lookupChannelIds, normalized);
    ...
    if (found == null) {
      found = await this.findCustomerByRaw(c, merchantId, lookupChannelIds, input.rawValue);
    }
    const customerId =
      found ?? (await this.createCustomer(c, merchantId, input.displayName ?? null));
  ...
  async lookupIdentity(input: LookupInput): Promise<LookupResult | null> {
    ...
    const { rows } = await this.run<{ contactId: string; customerId: string }>(
      ...
      `SELECT c.id::text        AS "contactId",
              merchant.customer_survivor(cu.id)::text AS "customerId"
         FROM merchant.contact c
         JOIN merchant.customer cu ON cu.id = c.customer_id
        ...
        LIMIT 1`,
      [input.merchantId, lookupChannelIds, normalized],
    );
    const r = rows[0];
    ...
    const { rows } = await this.run<{
      ...
      `SELECT cu.name,
       ...
       LEFT JOIN LATERAL (
         ...
         FROM merchant.contact co
         JOIN umi.channel_type ch ON ch.id = co.channel_id
         WHERE co.merchant_id = cu.merchant_id
       ...
       LIMIT 1`,
      [merchantId, customerId, IdentityResolver.PHONE_FAMILY_KEYS, channelKey],
    );
    const r = rows[0];
    if (!r) return null;
    return {
  ...
  private async normalize(
    ...
    const { rows } = await this.run<{ n: string | null }>(
      c,
      `SELECT merchant.normalize_identity($1, $2) AS n`,
  ...
  private async findCustomer(
    ...
    const { rows } = await this.run<{ customerId: string }>(
      c,
      `SELECT merchant.customer_survivor(cu.id)::text AS "customerId"
        ...
        LIMIT 1`,
      [merchantId, channelIds, normalized],
    );
  ...
  private async createCustomer(
    ...
    const { rows } = await c.query<{ id: string }>(
      `INSERT INTO merchant.customer (merchant_id, name)
  ...
  private async ensureContact(
    ...
    const { rows: existing } = await c.query<{ id: string }>(
      `SELECT id::text AS id FROM merchant.contact
        WHERE merchant_id = $1::uuid AND customer_id = $2::uuid AND channel_id = $3::uuid
          AND normalized_value IS NOT DISTINCT FROM $4
        LIMIT 1`,
      [i.merchantId, i.customerId, i.channel.id, i.normalized],
    );
    ...
    if (existing[0]) {
      ...
      await c.query(
        `UPDATE merchant.contact
            SET updated_at = now(),
                verified = merchant.contact.verified OR $2,
                verified_via = CASE WHEN $2 AND NOT merchant.contact.verified
                                    THEN 'whatsapp_inbound' ELSE merchant.contact.verified_via END
          WHERE id = $1::uuid`,
    ...
    const { rows: primaryExists } = await c.query<{ n: number }>(
      `SELECT 1 AS n FROM merchant.contact
        WHERE merchant_id = $1::uuid AND customer_id = $2::uuid AND channel_id = $3::uuid
          AND is_primary LIMIT 1`,
      [i.merchantId, i.customerId, i.channel.id],
    );
    const isPrimary = primaryExists.length === 0;

    // normalized_value is NOT supplied: merchant.tg_contact_normalize derives it from the
    ...
    const { rows } = await c.query<{ id: string }>(
      `INSERT INTO merchant.contact
         (merchant_id, customer_id, channel_id, raw_phone_number, raw_value,
