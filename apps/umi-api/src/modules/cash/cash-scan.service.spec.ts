...
function make() {
  const qr = {
    verifyQRPayload: vi.fn().mockResolvedValue({ cardId: 'card-uuid', qrToken: 'tok', isWalletScan: false }),
  ...
  const repo = {
    tenantConfig: vi.fn().mockResolvedValue({ name: 'Kala', timezone: 'America/Mexico_City', lifecycleCopy: {}, birthdayRewardName: 'Café gratis' }),
    activeRewardConfig: vi.fn().mockResolvedValue({ id: 'rc1', visits_required: 10, reward_name: 'Café' }),
    ...
    performScan: vi.fn().mockResolvedValue({ total_visits: 6, visits_this_cycle: 4, pending_rewards: 0, balance_cents: 0, card_number: 'KAL-1' }),
  };
  const walletPass = { refreshCard: vi.fn().mockResolvedValue(undefined) };
  const email = { send: vi.fn().mockResolvedValue(null) };
  const svc = new CashScanService(qr as never, cards as never, repo as never, walletPass as never, email as never);
  return { svc, qr, cards, repo, walletPass, email };
}

const CARD = {
  id: 'card-uuid', card_number: 'KAL-1', qr_token: 'tok',
  total_visits: 5, visits_this_cycle: 3, pending_rewards: 0,
  balance_cents: 0, person_id: 'p1', display_name: 'Ana', normalized_email: null,
};

describe('CashScanService.scan — visit cycle', () => {
  let h: ReturnType<typeof make>;
  beforeEach(() => { h = make(); h.cards.findCard.mockResolvedValue(CARD); });
  ...
  it('earns a reward at the threshold visit (>=) and renders reward_earned moment', async () => {
    ...
    h.repo.performScan.mockResolvedValue({ total_visits: 6, visits_this_cycle: 0, pending_rewards: 1, balance_cents: 0, card_number: 'KAL-1' });
    const r = await h.svc.scan('t1', 'u1', { qrPayload: 'jwt', action: 'VISIT' });
    const arg = h.repo.performScan.mock.calls[0][0];
  ...
  it('sorts actions into BIRTHDAY→REDEEM→VISIT regardless of input order', async () => {
    ...
    await h.svc.scan('t1', 'u1', { qrPayload: 'jwt', actions: ['VISIT', 'BIRTHDAY_REDEEM', 'REDEEM'] });
    const arg = h.repo.performScan.mock.calls[0][0];
...
describe('CashScanService.scan — guards', () => {
  ...
  it('400 on invalid QR', async () => {
    ...
    await expect(h.svc.scan('t1', 'u1', { qrPayload: 'jwt', action: 'VISIT' })).rejects.toBeInstanceOf(BadRequestException);
  });

  it('404 when the card is not found', async () => {
    h.cards.findCard.mockResolvedValue(null);
    await expect(h.svc.scan('t1', 'u1', { qrPayload: 'jwt', action: 'VISIT' })).rejects.toBeInstanceOf(NotFoundException);
  });
  ...
  it('403 when scanning your own card', async () => {
    ...
    await expect(h.svc.scan('t1', 'u1', { qrPayload: 'jwt', action: 'VISIT' })).rejects.toBeInstanceOf(ForbiddenException);
  });

  it('429 on a second visit the same day', async () => {
    h.repo.visitedToday.mockResolvedValue(true);
    await expect(h.svc.scan('t1', 'u1', { qrPayload: 'jwt', action: 'VISIT' })).rejects.toMatchObject({ status: 429 });
  });
  ...
  it('400 redeem with no pending rewards', async () => {
    await expect(h.svc.scan('t1', 'u1', { qrPayload: 'jwt', action: 'REDEEM' })).rejects.toBeInstanceOf(BadRequestException);
  });
});
