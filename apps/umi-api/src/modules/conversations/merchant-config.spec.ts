...
describe('resolveVoiceConfig', () => {
  ...
  it('falls back assistant_name to the merchant name when no voice name set', () => {
    const v = resolveVoiceConfig(null, 'Kalala', 'merchant-1');
  ...
  it('maps a formal preset to its Spanish guidance', () => {
    const config: MerchantConfig = { voice: { tone_preset: 'formal' } };
    const v = resolveVoiceConfig(config, 'Kalala', 'merchant-1');
  ...
  it('falls back to the friendly default for an unknown preset key', () => {
    const config = { voice: { tone_preset: 'snarky' } } as unknown as MerchantConfig;
    const v = resolveVoiceConfig(config, 'Kalala', 'merchant-1');
    expect(v.tone).toBe(TONE_PRESETS.friendly.tone);
  });
  ...
  it('caps a long merchant-name fallback to the assistant_name bound', () => {
    const v = resolveVoiceConfig(null, 'B'.repeat(120), 'merchant-1');
    expect(v.assistant_name.length).toBe(60);
  });
});
