...
/**
 * Structured intent extraction. Verbatim port of
 * `whatsapp-handler/intent-extractor.ts` (behavior-fidelity carry-over,
 * preflight §7) — the Spanish rules, fuzzy synonym matching, clarification
 * heuristics, and confirmation overrides are preserved exactly. Only the
 * Anthropic call is rebound to the injected adapter.
 */
...
export class IntentService {
  ...
  async extractIntent(params: {
    ...
    if (
      shouldTreatAsConfirmationContext({
        currentState: params.currentState,
        pendingClarification: params.pendingClarification,
        draftCartSummary: params.draftCartSummary,
      })
    ) {
