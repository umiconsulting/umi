...
create table runtime.device_enrollment_request (
  ...
  device_id             uuid references tenant.device(id),
  created_at            timestamptz not null default now(),
  updated_at            timestamptz not null default now(),
  unique (business_id, idempotency_key),
  unique (business_id, decision_idempotency_key),
  constraint device_enrollment_request_branch_scope_fk
...
create index device_enrollment_request_active_idx
  on runtime.device_enrollment_request (business_id, branch_id, state, expires_at);
