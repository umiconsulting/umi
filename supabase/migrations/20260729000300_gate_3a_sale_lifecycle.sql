...
alter table tenant.pos_cart
  ...
  add column customer_id uuid references tenant.customer(id),
...
drop index tenant.pos_cart_active_operator_uidx;
create unique index pos_cart_active_operator_uidx
  on tenant.pos_cart(business_id,branch_id,operator_user_id)
  where lifecycle_state in (
    'building_cart','ready_for_checkout','recovered'
  );
create index pos_cart_lifecycle_history_idx
  on tenant.pos_cart(business_id,branch_id,lifecycle_state,updated_at desc,id);
create index pos_cart_customer_idx
  on tenant.pos_cart(business_id,customer_id)
  where customer_id is not null;

create or replace function tenant.pos_cart_lifecycle_guard() returns trigger
language plpgsql
as $$
begin
...
before update on tenant.pos_cart
for each row execute function tenant.pos_cart_lifecycle_guard();

comment on column tenant.pos_cart.lifecycle_state is
  'Gate 3A lifecycle state. Payment and receipt authority remain in checkout tables.';
comment on column tenant.pos_cart.customer_id is
  'Optional customer attachment only. Loyalty and customer editing remain out of scope.';
comment on column tenant.pos_cart.operator_user_id is
