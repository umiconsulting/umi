...
| 5A Dashboard operational completion     | Incomplete                 | 4A                     | Complete safe administrative commands     | Device-bound P0 mutations have no Dashboard session authority | Permission-scoped 21-domain read model, location isolation, API tests, Dashboard tests, and production build passed      |

The latest relevant revision is the Gate 5A incomplete closeout commit in PR #72.
PR #72 keeps Gates 3A through 4A complete. Gate 5A remains incomplete.
...
It adds effective-permission navigation and closes a location capability leak.
Device-bound administrative mutations remain open. Gate 6A is not authorized.
