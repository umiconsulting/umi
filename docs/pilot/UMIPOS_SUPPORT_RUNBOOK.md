...
| Sesión vencida o `401`        | la sesión deja de autorizar             | estado de sesión y reloj                   | autentica de nuevo y repite solo una lectura segura  | no reutilices el token                          | P1 si bloquea operación      |
| Permiso denegado o `403`      | la acción está prohibida                | rol, ubicación, permiso y aprobación       | corrige la asignación autorizada o usa otro operador | no amplíes el rol para evitar el control        | P1 si el permiso es correcto |
| Ubicación incorrecta          | contexto o datos inesperados            | negocio, ubicación, dispositivo y registro | detén la mutación y selecciona el contexto correcto  | no continúes una venta                          | P0/P1                        |
| Conflicto `409`               | el dato cambió                          | versión, comando y resultado original      | recarga, revisa y crea una acción nueva si procede   | no fuerces el estado                            | P1                           |
| Validación `400` o `422`      | datos rechazados                        | campos y unidades sin secretos             | corrige la entrada y vuelve a previsualizar          | no cambies datos directamente en PostgreSQL     | P1 según la acción           |
| Límite `429`                  | intento temporalmente bloqueado         | tiempo de espera y categoría               | espera el plazo indicado y conserva el contexto      | no cambies identidad ni multipliques intentos   | P1 si persiste               |
| Configuración ausente         | el proceso no inicia                    | precheck y nombre de variable              | configura el valor mediante el almacén de secretos   | no uses un valor de desarrollo                  | P0/P1                        |
| CORS, CSP u origen            | el navegador bloquea acceso             | origen público, HTTPS y proxy de confianza | corrige la configuración y vuelve a ejecutar smoke   | no desactives la protección                     | P1                           |
| Cookie segura                 | login no persiste por HTTPS             | dominio, `Secure`, proxy y reloj           | corrige TLS, dominio y proxy                         | no cambies a una cookie insegura                | P1                           |
| Provider de pago desactivado  | no existe autorización integrada        | modo del entorno y manifiesto              | usa efectivo o terminal manual dentro del alcance    | no habilites un mock como proveedor real        | P1 si era requisito          |
| Object storage desactivado    | una función opcional no está disponible | política y dependencia real                | mantén desactivado o valida el provider en Gate 13   | no añadas credenciales sin contrato             | P2 o P1 si era requisito     |
| Release incorrecto            | versión o checksum no coincide          | manifest, commit, imagen y esquema         | detén el despliegue y selecciona el RC certificado   | no continúes con RC1                            | P0/P1                        |
