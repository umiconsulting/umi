...
The prior research settled one axis: POS↔KDS is *sometimes* one app (Toast) and *just as
often* two purpose-built clients (Square, Lightspeed). The kiosk introduces a **second, cleaner
...
KDS, because the kiosk literally *is* the POS order-entry flow pointed at a guest.
...
- **Same app, a switchable mode — verbatim:**
  ...
  And: *"A user with manager or admin permissions can switch the device into and out of Kiosk
  Mode at any time using their passcode."*
...
- **Trust / isolation — a client lock, a PIN gate, and an OS lock-task, layered:**
  > To exit, press and hold the "powered by Toast logo," after which *"a manager or admin must
  > enter their passcode."* *"This prevents guests from closing the Toast POS app, whether
  > intentionally or by mistake."* Certain hardware ("22-inch Elo V4, 22-inch Toast Flex, and
...
- **Reuse is squarely on the POS↔kiosk axis.** The kiosk uses the same menu/catalog
  ...
  same kitchen: the kiosk keeps *"sending tickets to the KDS in the event that your devices
  enter Offline Mode during an outage"*
  ...
  order-*entry* client; the KDS remains the order-*queue* client downstream.
...
- **Trust / isolation — by construction, not by a documented lock.** Because the kiosk is a
  *different app* signed in with a *kiosk device code*, it structurally exposes only the
...
- **Reuse still on the POS↔kiosk axis at the data layer.** The kiosk shares the catalog and
  ...
  ([kiosk software](https://squareup.com/us/en/point-of-sale/restaurants/kiosk-software)). So Square agrees on the *family* (kiosk shares the POS
...
it shares the POS catalog, cart, checkout, and kitchen routing. Whether it is a *mode of the
POS app* (Toast) or a *separate app on the same backend* (Square, Clover) follows the exact
...
- **The POS/server receives a read-only notification of readiness.** "Any time an order is
  ...
  push to the POS**, with the fulfil *command* reserved to the KDS role.
...
POS holds order *commands* — but for the takeout/delivery **order lifecycle**, a front-of-house
job, not kitchen prep.

## 5. Dimension C — table / floor / seating is a front-of-house surface, never a KDS one

### 5.1 Toast — tables are FOH; the KDS only *shows* coursing, never manages it
...
- **Advanced reservations/waitlist/host-stand is a *separate FOH app* — a notable finding.**
...
- **The KDS is a read-only consumer of table data, not an owner.** "If your restaurant uses a
  ...
  *reflects* coursing of items already sent, it never manages the floor
...
floor/table management on the kitchen display.** The KDS only *reads* coursing/table labels to
...
- **KDS mode is a device-level "Primary Mode," set in Device Setup — verbatim:**
  ...
  from the individual's role. A login authenticates *who*; the device decides *which surface*.
...
- **Capabilities are a SECOND, independent gate (the user's permissions).** Even where POS
  ...
  the app *contains* that code. Toast's model layers **device Primary Mode (which surface)** on
...
- **INFERENCE (labelled):** because the surface is device-locked and mode-switching plus each
  ...
  is closed by *server-side authorization*, not by app separation. The client *contains* the POS
  code (that is the residual attack surface of any fused binary); what it *permits* is scoped by
...
- **NOT VERIFIED:** whether Toast KDS requires a per-employee passcode to bump/fulfil each ticket
  ...
  *explicitly* says "a KDS-mode device hides the POS ordering UI." Both are inferred from the
...
privilege-escalation surface **zero by construction** — this is the security argument *for*
separate clients, and the mirror image of the code-reuse argument *for* fusion.

### 6.3 The general principle

Toast confirms the principle Umi should adopt: **the device role is an admin-provisioned property
of the device/session, and a login only authenticates *who*, never *which surface* — while
sensitive capabilities are independently permission-scoped.** That is precisely a *server-enforced*
...
**Dimension D verdict:** client fusion *does* create a residual privilege-escalation surface that
...
- (Customer-facing display — Square hardware has a customer display for payment; a standalone
  guest-display *client* was not verified here — **NOT VERIFIED**.)
...
prep commands on an unattended station — and the vendors keep the *command* there: Toast's POS
...
1. **The strongest fuse case is POS↔kiosk, and it is under-exploited by the current ADR.** The
   ...
   role** with near-zero new order-entry code. This is a *reinforcement* of the ADR's
...
5. **Fusion adds a privilege-escalation surface; close it on the server, as Toast does.**
   A single binary that renders POS *and* KDS *and* kiosk carries the cashier code (orders,
   drawer, refunds) onto the kitchen and public devices — a surface that Square's separate KDS
   and Kiosk apps avoid *by construction*. Toast keeps this safe with a two-layer,
   ...
   **server-provisioned and admin-locked**, and every sensitive action must re-check the user's
   permission server-side — so that a barista PIN on a KDS or kiosk device authenticates *who*,
   never unlocks *which surface* or *which capability*. Do this and fusion is as safe as
...
- **Customer/guest-display client architecture** for both Toast and Square (as a standalone
  in-house *client*, distinct from the payment-facing customer display) was **not** the focus
...
- **Toast KDS login/isolation (Dimension D):** whether the Toast KDS requires a per-employee
  ...
  pages read; and no page read *explicitly* states "a KDS-mode device hides the POS ordering
