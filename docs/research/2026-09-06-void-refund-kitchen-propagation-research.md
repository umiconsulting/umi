...
- **Timing / state gate.** Payment capture is the wall between void and refund:
  ...
  One nuance about the *fulfillment* gate rather than the money gate: an item-level adjustment
...
- **Model prep state as the switch, not payment state.** Square's primary rule — "not yet
  ...
  *second, independent* gate for the money leg — see the last bullet.) [Evidence: Square
...
- **Payment/settlement is a real second gate — decide Umi's threshold deliberately.** Toast:
  ...
  cheap reversal (void); post-settlement = refund. This gate governs the *money*; the *KDS*
  propagation gate above is separate and governs the *food*. [Evidence: Toast + Clover primary.]
...
- **Toast:** the exact "void food not made / comp food already made — because void restores
  ...
  *single line* (vs a whole check) is inferred from the check-level quote.
