...
value of asking the vendors is that Toast and Square between them document the _full_
...
lock voids to managers by _removing_ the void permission from the server job
([VENDOR-PRIMARY](https://support.toasttab.com/en/article/Voiding-Items-Payments-and-Checks)).
...
- **KDS core: bump / recall / complete** (item- and order-level) — the irreducible KDS
  ([Square](https://squareup.com/help/us/en/article/8171-complete-orders-with-square-kds)). _(A
  café may substitute a single bar ticket printer for a KDS at first —
  [Square](https://squareup.com/help/us/en/article/5194-print-order-tickets).)_
...
- **86 → KDS visible alert:** propagation to **POS + online + third-party** is verified for both
  Toast and Square; a visible "sold-out" alert _on the KDS_ is **NOT VERIFIED** (the documented
...
- **Toast item-level dayparting:** confirmed _limitation_ — menu availability is "a menu-level
