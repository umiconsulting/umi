...
**Dashboard deploy model — hardened 2026-06-26.** The cookie cutover failed in production ("can't select tenants") because the Vercel build was shipping the local-dev `.env` (`VITE_AUTH_MODE=local`, no `VITE_API_BASE`). The SPA therefore compiled in same-origin/Supabase mode and 401'd against `api.umiconsulting.co`. The bug was not in the cutover — it was that build config had two sources and the wrong one won. Fixed and closed:

- **Production config lives in the Vercel project's environment variables**, set as _plain_ (5 × `VITE_*`: `VITE_AUTH_MODE=cookie`, `VITE_API_BASE`, `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_BUSINESS_SLUG`). The local `.env` is excluded from builds by [`apps/umi-dashboard/.vercelignore`](../../apps/umi-dashboard/.vercelignore), which carries the explanation so the next person does not re-add it.
- **The Vercel project points at this monorepo** (`umiconsulting/umi`, root `apps/umi-dashboard`, production branch `main`), with **Install Command forced to `npm install`** — the root `pnpm-workspace.yaml` made Vercel auto-select pnpm, which crashed. Deploys are git-based: push `main` → production, push a branch → preview.
- **Staging:** branch `staging` → `staging-dashboard.umiconsulting.co`. Cookie auth works there because it is under `.umiconsulting.co`; `CORS_ORIGINS` lists both origins. Workflow: branch off `main` → verify on staging → merge.
- **Auth correctness:** login errors no longer render `[object Object]` — umi-api wraps errors as `{error:{message}}`, so the SPA reads them through an `errMessage()` helper in `src/lib/config.js`.
- **OPEN (optional).** `CORS_ORIGINS` in `main.ts` is still an explicit comma-separated allowlist, so every new first-party subdomain costs a backend env edit. A `*.umiconsulting.co` regex would end that — the auth cookie already spans every subdomain via `Domain=.umiconsulting.co`. `cash.` and `kds.` were the cases named at the time; the POS and device surfaces add more. Native KDS (iOS) cannot use browser cookies at all and needs a token path when it is ported.
