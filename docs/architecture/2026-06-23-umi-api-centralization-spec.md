...
**Status (2026-06-30):** Code-complete in `apps/umi-api/src/modules/leads/*`. **294/294 vitest** (25 new leads unit tests: scoring, sequence idempotency + flag-gating, webhook HMAC, contact) + tsc + build green. **Dormant** — public contact/diagnostic/webhook routes are mounted and live, but the background email-sequence tick stays OFF until `LEADS_SEQUENCE_ENABLED` is flipped (so the landing page's own cron stays the sole sender during the dual-run window).
**Realized design (vs. the original plan):**
