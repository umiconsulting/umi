...
_absorption into the dashboard_ is designed here (§8), but the app itself is left untouched
until cutover.
...
senior engineer would, it means _one coherent rule-set_, applied idiomatically per
...
Cross-app _code_ extraction (a real `packages/ui`, `packages/api-client`) is a larger,
riskier program and is **explicitly deferred**; this blueprint standardizes _conventions_
...
- History (memory): Vercel's dashboard project was forced to `npm install` to dodge the
  pnpm workspace. So the repo _says_ pnpm and _does_ npm. This must be resolved one way.
...
> by dominance, not vote: per-app lockfiles are _architecturally disqualified_ (no workspace ⇒ no
> shared `packages/*` ⇒ the contract/token/config sharing this whole plan depends on is
> impossible). npm-workspaces reaches the same capability but is _strictly worse_ — pnpm's
...
> Command flip, not an engineering trade. Only an _external_ constraint (a Vercel policy
...
> CodeRabbit skips >150-file PRs so a stacked mega-PR gets _zero_ review; per-phase = independent
> revert + bisect).

- **One package manager: pnpm** (the root already commits to it). One **`pnpm-lock.yaml`
  at the root**; delete all five per-app `package-lock.json`. _(pnpm workspaces exist
  precisely to give a single lockfile; Turborepo reads the pnpm workspace graph.)_
...
- umi-api (Nest) currently uses relative-only; it adopts `@/*` for cross-module imports so
  the _whole repo_ shares one alias (§5 debates whether Nest is exempt).
...
- **Model A — global kebab-case.** _Every_ file in the repo is kebab-case, including React
...
- **Model B — framework-idiomatic (one rule-set).** File name always kebab-case **except**
  a file whose _default export is a React component_ is PascalCase (matching the component
  ...
  - Con: two casings coexist (PascalCase components alongside kebab everything-else) — though
    that pairing _is_ the documented React convention, not an inconsistency.
...
## 4. Critique — the argument _against_ this blueprint
...
1. **"Same format everywhere" can become dogma that fights the tools.** Model A's global
   ...
   the moment someone runs a scaffolder. → _Model B is the senior default; adopt A only if
   the owner explicitly values absolute uniformity over ecosystem fit._
2. **Big-bang renames are pure risk with near-zero product value.** Renaming 100+ files
   ...
   a broken prod deploy. → _Naming is the LAST phase, per-app, each behind its own green
   build + CI, never bundled with config changes._
3. **Forcing umi-api onto `@/*` fights NestJS.** The Nest CLI, docs, and every generator
   ...
   cross-repo tidiness for real friction with the framework. → _Let Nest keep relative
   imports + dotted-kebab; "consistency" means "idiomatic within each stack," which §5
   Model B already encodes._
4. **pnpm consolidation has a known Vercel scar.** The dashboard was moved to `npm install`
   ...
   → _Resolved (§2.1): pnpm, done properly — root lockfile + Vercel Install Command per
   project. The half-state (declares pnpm, ships npm lockfiles) was the actual bug._
5. **hyper-strict tsconfig as the floor may surface latent type errors** in cash/logs/api
   that currently compile. → _Land the base config in "loose-compatible" mode, then ratchet
   each strict flag per app behind its own green `typecheck`._
6. **dashboard JS→TS is a real migration, not a rename.** It can regress a live app for
   cosmetic gain. → _Defer; treat as its own project, not part of standardization._
7. **Formatter adoption reflows every file = one giant noisy diff.** → _Land Prettier in a
...
- Turborepo — _Structuring a repository_ & _Managing dependencies_ (turborepo.dev): `apps/`
...
- pnpm workspaces (official): a workspace has **one lockfile** at the root — the stated
  reason to use it.
- Next.js — _File-system conventions_ (nextjs.org): special files (`page`, `layout`,
...
- pnpm workspaces are designed around a _single_ root lockfile; five per-app npm lockfiles
...
- Next.js mandates specific lowercase file names for routing; component _file_ casing is
...
- **dashboard JS→TS** migration — _elevated to P3 (§8.2/§9.8) by the product direction; kept here only as a pointer_.
...
>   does not retire — it _slims to its customer/wallet half_; only its operator/admin half
...
The folder layout (`apps/umi-*`) is flat and treats all six as peers. The _product_ reality
is three tiers:
...
- **Native client:** `umi-kds` (Swift/iPad). HTTP-only client of umi-api; shares _no_ code
...
So the owner's instinct — "once cash folds in, is KDS the only other app?" — is right _for the
operator surface_: the platform becomes api+dashboard, and **KDS is the one remaining
...
- **`packages/contract`** — the API's request/response DTOs + route paths as TypeScript types,
  **owned by umi-api** (source of truth) and **imported by umi-dashboard** (client). Change a
  ...
  compiler, which is _stronger_ than living in the same directory. Today the dashboard is JS
...
**Folder grouping** (`apps/platform/{api,dashboard}`) is _optional and deferred_: it would make
...
**Consequence:** because the dashboard is becoming _the_ product front-end (and will host a
...
### 8.3 The dashboard's new form — absorbing cash's _operator_ half
...
- _*Why cash.* is frozen by physics:_* printed, in-store QR codes point at
...
- **Cookie landmine (the one real hazard):** the auth cookie is `Domain=.umiconsulting.co`, so
  it is sent to _every_ subdomain. The operator session and the customer-wallet session must
...
_*Arguments FOR the split (app.* console + cash._ customer, cash-as-section):**

- The two surfaces are genuinely different products — different auth, audience, UX, and deploy
  cadence. umi-cash today _conflates_ them; the split fixes an existing smell, it doesn't invent one.
...
**Reason (the through-line):** we are not _imposing_ a split — we are _separating two products that
were wrongly merged_, under one constraint that removes the main degree of freedom (the QR-pinned
...
Same lens as §8, applied to _every_ app. Axes: **audience**, **does it own data/secrets**
...
But the same app _also_ calls umi-api: `apiUrl("/api/contact")` and `apiUrl("/api/diagnostic")`
...
matter for now." This is the _cleanest_ security remediation: removing the app removes the
...
_Ideal future observability tool (when rebuilt):_
...
- **Data access:** **NO direct DB from the browser, ever.** Reads _only_ through **umi-api
...
flagship console is the _least_-engineered visually (no design system while it's about to host
a POS).
...
**framework-neutral CSS custom properties** (so the token layer does _not_ force Tailwind onto
...
— _documented fact_. But it is **NOT a ratified W3C Standard nor on the W3C Standards Track**;
...
**One rule fixes most of it:** _only umi-api owns data and secrets; every frontend is a **typed
client of the api** and a **consumer of shared tokens**._ Target packages:
...
**FOR:** boundary surgery is _lower_-risk than restructuring — every app keeps its audience; it
...
1. _The logs rebind needs api observability endpoints that may not exist yet_ → could balloon.
...
2. _Shared packages add build-graph + turbo wiring complexity._ **Fix:** that is precisely what
   the already-declared pnpm+turbo exist for (§2.1); this is paying down, not adding, debt.
3. _Deleting landing's lead engine could break a dormant sequence._ **Fix:** it's dormant AND
   duplicated AND api already owns it AND landing already calls the api path — delete after a
   one-shot verification, not blind.
4. _"Don't merge apps" may under-reach — should logs be a super-admin console route?_ **Fix:**
   no — internal-vs-tenant is an audience boundary, same as operator-vs-customer; keep separate.
5. _Adopting DTCG on a hand-CSS dashboard is friction._ **Fix:** emit **CSS variables**, not a
...
secrets. Fix the boundaries first (and the logs credential leak _first of all_); the cosmetic
