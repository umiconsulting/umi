...
→ The H1 *pool reassignment* is **close to a no-op** for umi-api. Every `withTenant` (umi_app-pool) call site touches tables that stay in the RLS-enrolled `tenant` schema (cash/customers/tenants/staff/hours/voice) — all reachable by `umi_app` post-rename. **What survives of H1 is the SESSION *reshape* (F2), not a pool move.** Still add a live `umi_app`-pool `/auth/login` + KDS-pairing check to the go/no-go (cheap, guards regressions).
