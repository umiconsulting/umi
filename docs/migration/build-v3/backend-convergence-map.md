...
| `tenant.open_hours`                                                                        | **jsonb column on `tenant.business`**, not a table (parity gate caught this; map's earlier "column false-positive" was wrong — the backend uses it as a TABLE: `FROM/INSERT INTO/DELETE FROM`, one row per weekday) | rewrite `business-hours.repository.ts` + cash-scan open-hours join to jsonb ops                                                                                |
