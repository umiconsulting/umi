...
|---|---|---|---|
| D1 | **Connection role reconciliation** | App pools connect as a role that is `pg_has_role(current_user,'api','member')`, **not** `postgres`/`service_role`; `rolsuper=f, rolbypassrls=f` on the app+readonly pools; only the worker pool is BYPASSRLS. Boot guard aborts otherwise. | ✅ **Names decided: `api`/`worker` (canonical).** The role is embedded in `DATABASE_URL_APP`/`_WORKER` (env), so cutover points those at `api`/`worker` — an **env change, not code** (`pg.service.ts` doc updated). Still add the boot guard asserting the pool role. |
