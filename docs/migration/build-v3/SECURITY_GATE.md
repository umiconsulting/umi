...
| D3 | **Pooler / SET LOCAL isolation** | `app.current_business` is set via `set_config(..., true)` **inside the same transaction** as the query (never a session `SET`); an integration test proves tenant A's pooled connection cannot read tenant B after reuse. | Supabase fronts with Supavisor (transaction pooling). Also: **no tenant-scoped request read may run on the BYPASSRLS worker pool.** ⚠️ **Sanctioned exceptions** (reconcile here, they are not new breaches): the auth substrate is worker-only *by design* (D11), and the entitlement reads (`AuthRepository.productStatus`, `TenantsRepository.loadProducts`) run on the worker pool because `umi.*` is SEALED from `api`. Both carry an explicit `business_id` predicate. The rule means *no UNSCOPED tenant read*; an explicit-predicate read on worker is the documented pattern. |
...
- **A grant cannot be revoked, only deleted** — `umi.user_role` has no `status` column (the build-v2 `tenant_access.status='active'` filter had nowhere to go), so disabling someone's access means removing the row. Acceptable at this size, but it leaves no audit trail of *why* access ended and no "suspended" state. Add `status` if access changes ever need to be reviewable.
- **A platform-wide grant is invisible to `api`** — `umi.user_role`'s policy is `business_id = umi.current_business()`, which a NULL `business_id` (the super_admin grant) can never satisfy. Intentional: the auth queries that read it run on the worker pool. Anything reading grants on the request path would silently not see platform grants.
