...
--
-- VERDICTS
--   comms.conversations       -> MAP  merchant.conversation
--   comms.messages            -> MAP  merchant.message (+ runtime.message_embedding)
...
--   comms.knowledge_documents -> EMPTY (would MAP merchant.knowledge_document)
--   comms.knowledge_chunks    -> EMPTY (would MAP merchant.knowledge_chunk)
...
-- ID CONTINUITY (verified): conversations.tenant_id ∈ merchant.merchant (0 miss);
--   conversations.person_id ∈ merchant.customer (0 miss, all 11 non-null).
-- ROLES (verified): only 'user'/'assistant' → CASE never yields NULL direction/sender.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1) merchant.conversation  <- comms.conversations   (11 rows)
...
--      selected_location_id (0 populated; merchant.conversation has no location column).
...
insert into merchant.conversation
  (id, merchant_id, customer_id, channel_id, status, outcome,
...
-- ---------------------------------------------------------------------------
-- 2) merchant.message  <- comms.messages   (1376 rows)
...
--      (merchant.message is scoped via conversation, no merchant_id column),
...
--        (select count(*) from merchant.conversation)  as dst_conv;                 -- 11 = 11
-- select (select count(*) from comms.messages) as src_msg,
--        (select count(*) from merchant.message)  as dst_msg;                       -- 1376 = 1376
...
-- select direction, sender, count(*) from merchant.message group by 1,2;            -- inbound/customer, outbound/bot
-- select count(*) filter (where provider_message_id is not null) from merchant.message;  -- 616
-- select count(*) filter (where external_ref is not null) from merchant.conversation;    -- 11
