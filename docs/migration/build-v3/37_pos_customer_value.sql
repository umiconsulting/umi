...
alter table merchant.loyalty_gift_card
  ...
  add column masked_code text,
  add column currency char(3) not null default 'MXN' check (currency ~ '^[A-Z]{3}$'),
  add column amount_cents bigint not null default 0 check (amount_cents>=0),
  add column ledger_sequence bigint not null default 0 check (ledger_sequence>=0),
  add column projection_version integer not null default 1 check (projection_version>0),
  add column version integer not null default 1 check (version>0),
  ...
  add column created_by_staff_id uuid references merchant.staff(id) on delete set null,
  ...
  add column redeemed_card_id uuid references merchant.loyalty_card(id) on delete set null,
  add constraint gift_card_merchant_id_uk unique (merchant_id,id),
  add constraint gift_card_customer_scope_fk foreign key (merchant_id,customer_id)
    references merchant.customer(merchant_id,id) on delete restrict;
update merchant.loyalty_gift_card set
  public_reference='GFT-'||id::text,
  code_hash=extensions.digest(code,'sha256'),
  masked_code='••••-'||right(code,4),
  code=encode(extensions.digest(code,'sha256'),'hex');
alter table merchant.loyalty_gift_card
  alter column public_reference set not null,
  alter column code_hash set not null,
  alter column masked_code set not null,
  drop constraint loyalty_gift_card_status_check,
  add constraint loyalty_gift_card_status_check check (status in (
    'created','inactive','active','suspended','depleted','expired','closed','restricted','redeemed','void'
  ));
create unique index loyalty_gift_card_hash_uidx
  on merchant.loyalty_gift_card(merchant_id,code_hash);
create unique index loyalty_gift_card_reference_uidx
  on merchant.loyalty_gift_card(merchant_id,public_reference);

create or replace function merchant.hash_gift_card_code() returns trigger
language plpgsql set search_path=pg_catalog,merchant,extensions as $$
begin
  new.public_reference:=coalesce(new.public_reference,'GFT-'||new.id::text);
  if new.code !~ '^[a-f0-9]{64}$' then
    if length(new.code)<12 then raise exception 'GIFT_CARD_CODE_INVALID'; end if;
    new.code_hash:=extensions.digest(new.code,'sha256');
    new.masked_code:='••••-'||right(new.code,4);
    new.code:=encode(new.code_hash,'hex');
  elsif new.code_hash is null or new.code<>encode(new.code_hash,'hex') then
    raise exception 'GIFT_CARD_CODE_PLAINTEXT_FORBIDDEN';
  end if;
  return new;
end $$;
create trigger loyalty_gift_card_hash_code
  before insert or update of code on merchant.loyalty_gift_card
  for each row execute function merchant.hash_gift_card_code();
...
alter table merchant.loyalty_gift_card_ledger
  ...
  add constraint gift_card_ledger_reason_check check (reason in (
    'issue','redeem','adjustment','migration_initial_load','load','expire','issued','loaded',
    'authorized','authorization_released','redeemed','refunded','reversed',
    'adjustment_increase','adjustment_decrease'
  ));
with ranked as (
