...
(just needs applying) or **NEEDS A CALL**. This is a *tracker*, not a plan — it
...
- **Where:** `2026-06-16-migration-plan.md` reads `umi_cash."…"` throughout
  ...
  (824, 826). Guardrail #3 says *"never the stale `umi_cash.*` copy — use a FRESH dump."*
...
- **Where:** `2026-06-16-migration-plan.md` §5.4, line 775: *"### 5.4 Optional cash
  append-only support."* The triggers (838–875) are correct but framed optional.
...
- **Original (wrong) recommendation:** "archive *every* pre-2026-06-16 plan."
...
- **Done (safe subset):** archived only the **3 genuinely-superseded *conflicting***
...
- **What:** the doc set actually contains **two program lineages** for the same
  ...
  **2026-05-14 → 2026-06-09 program** (`postgresql-platform-integration-plan` →
  ...
  labels the *"Active program driver."* The 06-16 runbook even cites "the workspace
