...
- [x] Phase 3 — Shared Foundations — **closed 2026-06-12**. S3.1 standalone PostgreSQL staging rehearsal passed core validation (2026-06-10). S3.3 tenant/capability endpoints validated against staging. **S3.2 closed:** dashboard deployed at `https://umi-dashboard.vercel.app` under `umiconsulting-dev` on Vercel; Git integration connected to `umiconsulting/umi-dashboard`; Vite build fixed (case-sensitive import `App.jsx` → `app.jsx`); API responding (DATABASE_URL needs port 6543 for Supabase pooler — one env var tweak).
- [x] Phase 4 — Backend Consolidation — **S4.1 executed 2026-06-10**: dashboard cut over to the single platform schema path. **S4.2 closed 2026-06-12**: `callKdsPairingLocal` deleted (200+ lines), all pairing routes through canonical `kds-pairing` edge function; `kds.device_pairing_requests` table created on Supabase; edge function verified end-to-end (`{"pairings":[]}` response). **S4.4 closed 2026-06-12**: 8 Vercel cron routes migrated to pg_cron schedules + job-worker processors (cash-cron.ts, 1214 lines); cron schedules registered matching vercel.json frequencies; end-to-end test passed (goal_proximity claimed → processed → completed); Vercel cron routes deleted (545 lines removed from umi-cash); pg_cron migration tracked as `20260613000003`. S4.1 verified end-to-end on deployed Vercel instance. **S4.3 blocked on owner go-ahead**: `rrkzhisnadfrgnhntkiz` untouchable until cutover-ready.
- [ ] Phase 5 — Monorepo Migration — **S5.1 executed + S5.2 rehearsed 2026-06-10; cutover blocked, all stoppers registered** in `docs/migration/audit-output/2026-06-10-phase-5-execution.md`. Root pnpm/Turborepo tooling committed (additive, inert for app npm workflows); history-preserving import mechanic proven in an isolated clone with exact commit conservation (279 = 7 root + 266 imported + 6 merges; Cash 218/218 via `git-filter-repo --to-subdirectory-filter`, `--no-local` clones mandatory). Hard stoppers: ST-1 Phase 4 incomplete (sequencing invariant), ST-2 no monorepo GitHub repo (owner naming decision), ST-3 zero Vercel projects under the consolidated identity → partially resolved: Cash is deployed under `umiconsulting-dev` (cash.umiconsulting.co); dashboard is now deployed; logs and landing still need Vercel projects., ST-4 offensive commit message `9df2c40` in pushed `umi-conversaflow` history — must be rewritten (`--replace-message`) before import and force-push-cleaned with owner go-ahead, ST-5 service_role rotation (S1.4) resolved 2026-06-12 via new secret key + vault update. Soft: landing `staging`-vs-`main` import branch, conversaflow branch topology, npm→pnpm conversion validation, renames batch. ST-10 resolved: umi-kds's uncommitted S2.3 work committed+pushed (`789bae5`).
