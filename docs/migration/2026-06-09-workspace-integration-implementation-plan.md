...
> **This plan is closed as a driver.** Phases 1–5 are done; Phases 6–7 were overtaken
> by the programs that followed — backend consolidation in
> [`2026-06-23-umi-api-centralization-spec.md`](../architecture/2026-06-23-umi-api-centralization-spec.md)
> and the schema program in [`build-v3/GATED_CUTOVER_PLAN.md`](build-v3/GATED_CUTOVER_PLAN.md),
> which is the live driver. Kept for the execution record below.

- [x] Phase 1 — Stabilization — **closed 2026-06-12**. S1.4 executed by creating a new Supabase secret key (`sb_secret_...`) and updating the existing Vault entry via `vault.update_secret` — approach changed from the original JWT-roll plan because the current Supabase JWT Signing Keys UI warns that rotation would break 5 edge functions (`zettle-oauth-setup`, `embed-backfill`, `order-status-webhook`, `v2-synthetic-eval`, `kds-command`). The new secret key achieves the same outcome: the hardcoded JWT in git history (`20260417130000_add_job_worker_cron.sql`) is now a dead credential; cron/trigger code reads from Vault at runtime. All other steps (S1.1 workspace root versioning, S1.2 Phase 4F migration, S1.3 semantic search restore, S1.5 adapter re-convergence, S1.6 uncertainty burn-down) were complete per the 2026-06-10 execution evidence.
...
- [x] Phase 3 — Shared Foundations — **closed 2026-06-12**. S3.1 standalone PostgreSQL staging rehearsal passed core validation with documented synthetic-family count deltas (2026-06-10); S3.3 tenant/capability endpoints validated against staging. **S3.2 closed:** the dashboard deployed at `https://umi-dashboard.vercel.app` under the `umiconsulting-dev` Vercel account, Git integration connected, Vite build fixed (the case-sensitive import `App.jsx` → `app.jsx`), and the API responding — `DATABASE_URL` needs port **6543** for the Supabase pooler, which was the whole of the remaining gate. See `docs/migration/audit-output/2026-06-10-phase-3-execution.md`.
- [x] Phase 4 — Backend Consolidation — **S4.1 executed 2026-06-10**: dashboard cut over to the single platform schema path (zero `PLATFORM_TRANSITION_SCHEMA` refs; legacy branches and dead helpers deleted; Prisma schema remapped to `dashboard_compat` views, no longer the Cash duplicate); verified by 28-endpoint matrix + write flows + browser walkthrough against the staging replay; two replay gaps scripted (`008_dashboard_compat_core.sql`, `kds.device_pairing_requests` → `005`). Evidence: `audit-output/2026-06-10-phase-4-1-dashboard-cutover.md`; verified end-to-end on the deployed Vercel instance once S3.2 closed. **S4.2 closed 2026-06-12:** `callKdsPairingLocal` deleted (200+ lines), all pairing routed through the canonical `kds-pairing` edge function, `kds.device_pairing_requests` created, edge function verified end-to-end. **S4.4 closed 2026-06-12:** 8 Vercel cron routes migrated to pg_cron schedules + job-worker processors (`cash-cron.ts`), schedules registered at the `vercel.json` frequencies, one journey tested end-to-end (goal_proximity claimed → processed → completed), and the Vercel cron routes deleted (−545 lines from umi-cash); tracked as migration `20260613000003`. **S4.3 closed by the unified-DB launch (2026-06-20):** the curated merge of both production databases went live on the platform Supabase project, `cash.umiconsulting.co` cut over, and `rrkzhisnadfrgnhntkiz` became the frozen rollback rather than the live database.
- [x] Phase 5 — Monorepo Migration — **closed 2026-06-20 by a different mechanic than the one rehearsed.** The monorepo is live at `umiconsulting/umi` as a **fresh snapshot** of the six apps under `apps/*`, rooted at `e229116` (2026-06-10) with no per-app history imported. That choice resolved the two hard stoppers at once: ST-2 (no monorepo repo) by creating it, and **ST-4 by construction** — the offensive commit `9df2c40` lives only in the old `umi-conversaflow` history, which was never imported and is absent from this repository, so no history rewrite was ever needed. The cost is the one the snapshot chose to pay: per-app history stayed behind in the original repos. ST-3 resolved as the Vercel projects were created (dashboard, cash, landing). The rehearsal below is kept because its evidence is still the reference for any future history-preserving import. Original record: **S5.1 executed + S5.2 rehearsed 2026-06-10; cutover blocked, all stoppers registered** in `docs/migration/audit-output/2026-06-10-phase-5-execution.md`. Root pnpm/Turborepo tooling committed (additive, inert for app npm workflows); history-preserving import mechanic proven in an isolated clone with exact commit conservation (279 = 7 root + 266 imported + 6 merges; Cash 218/218 via `git-filter-repo --to-subdirectory-filter`, `--no-local` clones mandatory). Hard stoppers: ST-1 Phase 4 incomplete (sequencing invariant), ST-2 no monorepo GitHub repo (owner naming decision), ST-3 zero Vercel projects under `juanclpzq` (nothing to repoint; Cash crons under unknown identity), ST-4 offensive commit message `9df2c40` in pushed `umi-conversaflow` history — must be rewritten (`--replace-message`) before import and force-push-cleaned with owner go-ahead, ST-5 service_role rotation (S1.4) resolved 2026-06-12 via new secret key + vault update. Soft: landing `staging`-vs-`main` import branch, conversaflow branch topology, npm→pnpm conversion validation, renames batch. ST-10 resolved: umi-kds's uncommitted S2.3 work committed+pushed (`789bae5`).
