...
- [ ] Phase 4 — Backend Consolidation — **S4.1 executed 2026-06-10**: dashboard cut over to the single platform schema path (zero `PLATFORM_TRANSITION_SCHEMA` refs; legacy branches and dead helpers deleted; Prisma schema remapped to `dashboard_compat` views, no longer the Cash duplicate); verified by 28-endpoint matrix + write flows + browser walkthrough against the staging replay; two replay gaps scripted (`008_dashboard_compat_core.sql`, `kds.device_pairing_requests` → `005`). Evidence: `audit-output/2026-06-10-phase-4-1-dashboard-cutover.md`. The "deployed backend" aspect stays gated by S3.2's missing Vercel project/staging secrets. S4.2 next (rotate the service_role key first — S1.4). **S4.3 blocked on owner go-ahead** per the 2026-06-10 directive: `rrkzhisnadfrgnhntkiz` (only real production DB) untouchable until cutover-ready; the read-only re-dump is the first sanctioned contact. S4.4 note: deleting Vercel cron routes touches the Cash app deployment — needs the same owner approval.
