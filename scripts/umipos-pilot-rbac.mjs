...
function generatedSql() {
  ...
  // The two always-seeded keys plus the ones the matrix adds, as one list, because
  // the guarded insert above needs the SAME list in both of its branches.
  const permissionValues = [
    `  (${quote('insights.read')},${quote('Read merchant operational insights')})`,
    `  (${quote('merchant.manage')},${quote('Manage one merchant business profile')})`,
